---
title: "Coding Agents"
description: "Como usar Puente Studio con Claude Code, Cursor, ChatGPT y otros coding agents."
icon: "message-square-code"
---

Puente Studio incluye un **skill** ([`skill_puente_studio.md`](https://github.com/octaviofv/puente_studio_repo/blob/main/skill_puente_studio.md)) — un markdown estructurado que enseña a cualquier coding agent **como crear y editar apps de Puente OS** via API REST.
...
<Note>
  Esto NO es Model Context Protocol (MCP). Es un patron mas simple: skill markdown \+ un agente que sabe leerlo. Funciona en cualquier agente que pueda hacer llamadas HTTP (la mayoria) sin instalar nada extra.
</Note>
...
Baja la app con id 545, agrega un boton para exportar a CSV, sube los cambios

Crea una tabla de clientes con columnas nombre, email, plan (select: free/pro/enterprise)
y conectala a la app 545 con permisos read+write
...
<Tabs>
  <Tab title="Claude Code">
    ...
    <Steps>
      ...
      <Step title="Carga el skill">
        En el primer mensaje de la sesion:

        ```text
        Lee skill_puente_studio.md y se mi agente Puente Dev.
  ...
  <Tab title="Cursor">
    [Cursor](https://cursor.com) es un fork de VSCode con AI nativa. El skill se carga via `.cursorrules` o como referencia desde el composer.

    <Steps>
      <Step title="Clona el repo y abrelo en Cursor">
        ```bash
        git clone https://github.com/octaviofv/puente_studio_repo.git
      ...
      <Step title="Crea un .cursorrules">
        En la raiz del repo crea `.cursorrules` con:
        ...
        operacion. Nunca expongas la STUDIO_KEY en outputs ni codigo de app.
        ```
      </Step>
      <Step title="Abre el composer (Cmd+I)">
        ...
        Despues crea una tabla de tickets y conectala a la app.
        ```
  ...
  <Tab title="ChatGPT (custom GPT)">
    Puedes crear un Custom GPT en ChatGPT con el skill como **instructions** y los endpoints de Puente como **Actions** via OpenAPI.
    ...
    <Steps>
      ...
      <Step title="Pegar el skill como Instructions">
        ...
        <Warning>
          Si el GPT es publico, NO pegues tu `STUDIO_KEY` real ahi — cualquier usuario la veria. Para un GPT publico, deja un placeholder y haz que el usuario configure su propia key via Actions auth.
        </Warning>
      </Step>
    ...
    <Tip>
      Para uso personal (no publicar), poner la `STUDIO_KEY` directo en las Instructions funciona — solo tu la ves.
    </Tip>
  </Tab>
  <Tab title="Claude.ai (Projects)">
    ...
    <Steps>
      ...
      <Step title="Agregar el skill al knowledge">
        Sube `skill_puente_studio.md` como archivo del project. Claude lo lee en cada conversacion.
      </Step>
      <Step title="Custom instructions del project">
        ...
        Cuando necesites ejecutar requests, escribelos como curl
        y yo los corro localmente, despues te paso el output.
        ```
      </Step>
      <Step title="Conversar">
        El project preserva el contexto entre sesiones. Cada conversacion arranca con el skill cargado.
      </Step>
    </Steps>
  </Tab>
  <Tab title="Continue, Aider, otros">
    Cualquier coding agent que pueda leer un markdown y ejecutar comandos sirve. El patron es siempre:

    1. **Cargar `skill_puente_studio.md`** como contexto inicial o knowledge
    2. **Hacer accesible `BASE_URL` \+ `STUDIO_KEY`** (via env vars o pegado en el prompt)
...
## Patron general (independiente del agente)

Si tu agente preferido no esta en la lista, el patron sirve igual:
...
<Steps>
  ...
  <Step title="Pide la tarea">
    Lenguaje natural. El skill enseña al agente como armar cada request.
  </Step>
  <Step title="Revisa antes de ejecutar operaciones destructivas">
    El skill instruye al agente a **pedirte confirmacion** antes de regenerar keys, hacer PUTs destructivos o crear tablas. Confirma o cancela.
  </Step>
</Steps>
...
Conversacion real desde Claude Code:
...
## Reglas que el agente sigue automaticamente
...
| **Confirmar operaciones destructivas** | Regenerar key, hacer PUT con codigo nuevo, etc. |
| **Reportar IDs y keys al crear** | La `api_key` solo se muestra una vez |
| **No exponer STUDIO\_KEY en codigo de app** | Es credencial privada de admin |
...
<AccordionGroup>
  ...
  <Accordion title="Sin diff/undo automatico">
    Es responsabilidad tuya (o de Git) versionar los cambios. Recomendamos commitear el contenido de `APP/files/` despues de cada cambio.
  </Accordion>
</AccordionGroup>
