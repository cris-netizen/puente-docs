...
<AccordionGroup>
  ...
  <Accordion title="¿Qué pasa si un nodo falla a mitad del flujo?">
    Depende del `on_error` del nodo. Por default (`Stop`), el flujo se detiene y los nodos siguientes quedan en `pending`. Con `Continue`, el error se registra y el flujo avanza al siguiente nodo. Puedes ver el error exacto en el panel de Resultados.
  </Accordion>
